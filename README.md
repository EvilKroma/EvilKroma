# <div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:4F46E5,50:7C3AED,100:A855F7&text=Dorian%20Deneuchatel&fontColor=ffffff&fontSize=50&fontAlignY=40&desc=Software%20Engineer%20%7C%20Full-Stack%20Developer%20%7C%20DevOps%20Enthusiast&descAlignY=60" />

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=26&pause=1000&color=8B5CF6&center=true&vCenter=true&width=900&lines=Software+Engineering+Student+at+ESIEE+Paris;Full-Stack+Developer;Java+%7C+React+%7C+Spring+Boot;Microservices+%26+Cloud+Architectures;DevOps+%7C+Docker+%7C+Kubernetes;Building+Scalable+Digital+Products" />

<br/>

![ESIEE Paris](https://img.shields.io/badge/ESIEE_Paris-Engineering_Student-6D28D9?style=for-the-badge)
![Computer Science](https://img.shields.io/badge/Computer_Science-Software_Engineering-4F46E5?style=for-the-badge)
![Alternance](https://img.shields.io/badge/Work_Study-Software_Engineer-7C3AED?style=for-the-badge)
![Location](https://img.shields.io/badge/France-Île--de--France-6366F1?style=for-the-badge)

<br/>

<a href="https://dorian-dev.fr">
<img src="https://img.shields.io/badge/Portfolio-Visit_Website-7C3AED?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/dorian-deneuchatel-27928325a">
<img src="https://img.shields.io/badge/LinkedIn-Professional_Profile-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:dorian.dev.contact@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact_Me-A855F7?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/EvilKroma">
<img src="https://img.shields.io/badge/GitHub-Profile-312E81?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<br/>

![](https://komarev.com/ghpvc/?username=EvilKroma&style=for-the-badge&color=7C3AED)
![](https://img.shields.io/github/followers/EvilKroma?style=for-the-badge&color=6366F1)
![](https://img.shields.io/github/stars/EvilKroma?affiliations=OWNER%2CCOLLABORATOR&style=for-the-badge&color=A855F7)

</div>

---

# About

Software Engineering student at **ESIEE Paris** with a strong background in full-stack development, enterprise software engineering, microservices architecture, and DevOps practices.

Currently working as an Information Systems Engineering Apprentice where I contribute to the modernization of industrial software systems by migrating legacy applications toward scalable microservice architectures while improving deployment automation, maintainability, and operational reliability.

My technical expertise spans across:

- Enterprise Java development
- Full-stack web applications
- Cloud-native architectures
- DevOps automation
- Containerized infrastructure
- API engineering
- AI-powered applications
- Software security

I enjoy building products that combine strong engineering principles with real business value, focusing on scalability, performance, maintainability, and user experience.

### Open To

- Software Engineering Opportunities
- Full Stack Development
- Java / Spring Boot Projects
- React Applications
- AI-Integrated Products
- DevOps & Cloud Engineering
- Open Source Collaboration
- Freelance Software Projects

---

# Tech Stack

## Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=java,cs,php,js,ts,python,dart,bash" />
</p>

## Frontend

<p align="center">
<img src="https://skillicons.dev/icons?i=react,nextjs,html,css,tailwind,flutter" />
</p>

## Backend & Databases

<p align="center">
<img src="https://skillicons.dev/icons?i=nodejs,spring,mysql,firebase,supabase" />
</p>

## Cloud, DevOps & Tooling

<p align="center">
<img src="https://skillicons.dev/icons?i=docker,kubernetes,jenkins,linux,git,github,postman,vscode" />
</p>

---

# AI / ML Expertise

| Domain | Proficiency | Details |
|----------|----------|----------|
| Generative AI | Advanced | LLM integrations, prompt engineering, AI workflows |
<!-- | AI Product Development | Advanced | AI-powered SaaS applications and automation | -->
<!-- | AI APIs | Advanced | Gemini, OpenAI, Anthropic integrations | -->
<!-- | Machine Learning Fundamentals | Intermediate | Model evaluation, datasets, pipelines | -->
| RAG Systems | Intermediate | Knowledge retrieval and contextual generation |
| AI Automation | Advanced | Workflow automation and business processes |
| AI Agents | Intermediate | Multi-step reasoning and autonomous workflows |
<!--  | AI-Assisted Development | Advanced | Copilot, Cursor, LLM-driven engineering | -->

---

# Featured Projects

---

# Experience

## Information Systems Engineering Apprentice - Roto France Impression (Maury Group)

**September 2025 - Present**

Contributing to enterprise software modernization initiatives focused on industrial information systems, microservices architectures, deployment automation, and infrastructure reliability.

### Scope of Work

- Modernization of legacy business applications
- Java and PHP enterprise development
- CI/CD implementation
- Kubernetes administration
- RFID systems integration
- Docker infrastructure management
- Linux system administration
- Production software optimization

**Skills**

`Java` `PHP` `Docker` `Kubernetes` `Linux` `Jenkins` `Microservices`

---

## Web Developer Apprentice - Nids Consulting

**September 2024 - May 2025**

Development of custom business applications with API integrations, deployment automation, maintenance, and client support.

### Scope of Work

- Full-stack development
- REST API integrations
- CI/CD deployment automation
- Software maintenance
- Client collaboration

**Skills**

`React` `Node.js` `REST APIs` `CI/CD` `Git`

---

## Web Development & Cybersecurity Intern - EasyVista

**May 2024 - June 2024**

Worked on improving application security posture through implementation of security policies and mitigation of frontend vulnerabilities.

### Scope of Work

- CSP implementation
- Nonce management
- XSS prevention
- Frontend security improvements

**Skills**

`Cybersecurity` `JavaScript` `CSP` `Web Security`

---

# Achievements

<div align="center">

| Recognition | Details |
|------------|------------|
| Engineering Student | ESIEE Paris |
| Full-Stack Developer | Professional Experience Across Multiple Stacks |
| DevOps Practitioner | CI/CD, Docker, Kubernetes |
| Entrepreneur | Freelance Development Projects |
| Security Awareness | Application Security & Hardening |
| Enterprise Experience | Industrial Information Systems |

</div>

---

# Certifications

## Cisco

![Introduction to Cybersecurity](https://img.shields.io/badge/Cisco-Introduction_To_Cybersecurity-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white)

## Additional

![React](https://img.shields.io/badge/OpenClassrooms-React-61DAFB?style=for-the-badge&logo=react&logoColor=black)

![TryHackMe](https://img.shields.io/badge/TryHackMe-Pre_Security-212C42?style=for-the-badge&logo=tryhackme&logoColor=white)

---

# GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=EvilKroma&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-streak-stats.herokuapp.com/?user=EvilKroma&theme=tokyonight&hide_border=true"/>

</div>

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=EvilKroma&layout=compact&theme=tokyonight&hide_border=true"/>

</div>

---

# GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=EvilKroma&theme=algolia&no-frame=true&column=7"/>

</div>

---

# Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=EvilKroma&theme=tokyo-night&hide_border=true"/>

</div>

---

# Contribution Snake

<div align="center">

<img src="[https://raw.githubusercontent.com/EvilKroma/EvilKroma/output/](https://raw.githubusercontent.com/EvilKroma/EvilKroma/output/)/github-contribution-grid-snake-dark.svg" />

</div>

---

# Current Focus

```yaml
Learning:
  - Advanced Spring Boot
  - Cloud Architecture
  - Kubernetes Operations
  - Software Design Patterns

Building:
  - AI-Powered Applications
  - Full Stack Platforms
  - Enterprise Web Systems
  - Mobile Applications

Exploring:
  - Machine Learning
  - Agentic AI
  - Microservices Architecture
  - Platform Engineering

Open_To:
  - Software Engineering Roles
  - Freelance Opportunities
  - Open Source Contributions
  - Technical Collaborations
```

---

# Connect

<div align="center">

<a href="mailto:dorian.dev.contact@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/dorian-deneuchatel-27928325a">
<img src="https://img.shields.io/badge/LinkedIn-Network-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/EvilKroma">
<img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://dorian-dev.fr">
<img src="https://img.shields.io/badge/Portfolio-Website-7C3AED?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

</div>

---

<div align="center">

*"Engineering reliable software, building scalable systems, and continuously learning."*

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=150&color=0:4F46E5,50:7C3AED,100:A855F7"/>
